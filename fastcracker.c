...
typedef struct {
    char sha256_input[65]; // SHA256 hash input
    int min_length;
    int max_length;
    bool found;
} ThreadData;
...
void* findRandomString(void* data) {
    ThreadData* threadData = (ThreadData*)data;

    char random_pass[MAX_STRING_LENGTH + 1];
    unsigned char hash_output[SHA256_DIGEST_SIZE];

    while (!threadData->found) {
        int length = randomLength(threadData->min_length, threadData->max_length);
        ...
        if (strcmp(sha256_hash, threadData->sha256_input) == 0) {
...
int main(int argc, char *argv[]) {
    ...
    //Added in for efficiency purposes with multiple cpu cores
    ThreadData threadData;
    strcpy(threadData.sha256_input, argv[1]);
